...
export async function getCurrentConditionsData(
  locationKey: string
): Promise<DataWithStatus<CurrentConditionsData>> {
  ...
  if (cacheData) {
    ...
    if (
      diffMilliseconds < 10 * 60 * 1000 &&
      currentTime.getHours() === cacheDatetime.getHours()
    ) {
      return {
...
// key - location
type CurrentConditionsCacheType = Record<
  string,
  Omit<CurrentConditionsData, 'datetime'> & { datetime: string }
>;
...
function saveDataToCache(
  ...
  const rawCacheData = localStorage.getItem(CURRENT_CONDITIONS_CACHE_KEY);
  const cacheData: CurrentConditionsCacheType = rawCacheData
    ? JSON.parse(rawCacheData)
    : {};

  localStorage.setItem(
    CURRENT_CONDITIONS_CACHE_KEY,
