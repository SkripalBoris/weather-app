...
export async function getHourlyForecastData(
  locationKey: string
): Promise<DataWithStatus<HourlyForecastData[]>> {
...
// key - location
type ForecastCacheType = Record<
  string,
  Array<Omit<HourlyForecastData, 'datetime'> & { datetime: string }>
>;
...
function saveDataToCache(
  ...
  const rawCacheData = localStorage.getItem(HOURLY_FORECAST_CACHE_KEY);
  const cacheData: ForecastCacheType = rawCacheData
    ? JSON.parse(rawCacheData)
    : {};

  localStorage.setItem(
    HOURLY_FORECAST_CACHE_KEY,
