...
export const DailyRange: FC<DailyRangeProps> = ({ min, max, lowest, highest, current }) => {
    ...
    const currentTempPointerStyle = useMemo(() => {
        ...
        return { '--offset': `${(current - lowest) / (highest - lowest) * 100}%` } as CSSProperties
    }, [current, lowest]);

    return <div
        className={s.dailyRange}
        style={rangeStyle}
    >
        <span className={s.dailyMin}>{min}°</span>
        <span className={s.range}>
            <span className={s.rangeMeter} />
            {current !== undefined && <div className={s.currentTemp} style={currentTempPointerStyle} />}
        </span>
